...
async function loadOrders() {
    ...
    for (var i = 0; i < data.orders.length; i++) {
        ...
        if (data.role === 'admin' && o.status === 'Новая заявка') {
            html += '<button class="btn btn-green" onclick="changeStatus(' + o.id + ', \'Услуга оказана\')">Выполнено</button>';
            html += '<button class="btn btn-red" onclick="openCancel(' + o.id + ')">Отменить</button>';
        }
        html += '</div>';
    }
